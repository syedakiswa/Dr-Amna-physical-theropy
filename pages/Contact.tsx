...
import { Phone, Mail, MapPin, Send, Sparkles, CheckCircle, AlertCircle, X } from 'lucide-react';
...
const ToastNotification: React.FC<{ toast: Toast; onRemove: (id: number) => void }> = ({ toast, onRemove }) => {
  ...
  return (
    <div
      className={`flex items-start gap-3 px-5 py-4 rounded-2xl shadow-xl border-l-4 ${s.bg} ${s.border} 
        animate-in slide-in-from-right-8 fade-in duration-300 max-w-sm w-full`}
    >
      {s.icon}
  ...
  <div className="fixed top-6 right-6 z-50 flex flex-col gap-3 pointer-events-none">
    {toasts.map((t) => (
      <div key={t.id} className="pointer-events-auto">
        <ToastNotification toast={t} onRemove={onRemove} />
      </div>
    ))}
  </div>
);
...
const Contact: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    addToast("✅ Appointment request submitted! Dr. Amna's team will contact you shortly.", 'success');
  ...
  const handleAskAI = async () => {
    if (!formData.message.trim()) {
      addToast('Please describe your condition in the message box first.', 'warning');
      return;
    }
    ...
    try {
      ...
      setAiResponse(
        response.text || 'Our specialists can definitely help. Please book a physical consultation for a detailed plan.'
      );
      addToast('AI insight ready! Check below the form.', 'success');
    } catch (error) {
      console.error('Gemini Error:', error);
      setAiResponse('We are ready to help! Please book a consultation to discuss your specific case with Dr. Amna.');
      addToast('AI response generated. Please review below.', 'warning');
    } finally {
      setAiLoading(false);
    }
  };
            ...
            <SectionWrapper delay={0.2} className="lg:col-span-2 space-y-8">
              <div className="bg-white p-10 rounded-3xl shadow-sm border border-medical-primary/5">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-medical-primary mb-2">
                      How can we help? (Describe symptoms)
                    </label>
                    <textarea
                      rows={4}
                      required
                  ...
                  {aiResponse && (
                    <div className="mt-6 p-6 bg-medical-secondary/10 border border-medical-secondary/20 rounded-2xl animate-in fade-in slide-in-from-bottom-2">
                      <div className="flex items-center space-x-2 mb-3">
                        ...
                        <span className="font-bold text-medical-secondary uppercase text-xs tracking-widest">
                          AI Clinical Assistant
                        </span>
                      </div>
                      <p className="text-medical-primary leading-relaxed text-sm">{aiResponse}</p>
                    </div>
                  )}
                </form>
              </div>
